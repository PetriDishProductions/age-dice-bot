...
client.on("message", (message) => {
  if (
    ...
    !message.content.startsWith(`${prefix}age`) ||
    message.author.bot
  )
    return;
  ...
  if (command === "age") {
    ...
    const color =
      Object.keys(checkForDoubles).length <= 2 ? "#ff0000" : "#00b0f4";
    const text =
      Object.keys(checkForDoubles).length <= 2
        ? `You rolled **${
            d1 + d2 + dragon + modiferValue
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setTitle(text)
      .setDescription(
        `\`(${d1} + ${d2}) + (Stunt Die: ${dragon}) + ${modiferValue} = ${
          d1 + d2 + dragon + modiferValue
        }\``
      )
      .setTimestamp()
    ...
    const helpText =
      "To roll dice, enter the command `!age` followed by the modifier, such as `!age +2`, `!age 2`, or `!age -1`. You cannot change the prefix at this time.";
